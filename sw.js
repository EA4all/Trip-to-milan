...
<div id="daily-tip-modal" class="modal-backdrop">
  ...
  <div id="floating-nav-container" class="md:hidden fixed bottom-0 left-0 right-0 z-40">
      ...
      <nav id="floating-nav" class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm shadow-[0_-2px_10px_rgba(0,0,0,0.1)] dark:shadow-[0_-2px_10px_rgba(0,0,0,0.3)] rounded-t-2xl">
        <div class="flex justify-around p-2 text-gray-700 dark:text-gray-300">
            ...
            <a href="#travel-journal" class="flex flex-col items-center text-xs gap-1 p-1 hover:text-brand-600 w-1/4">
                <i class="fa-solid fa-book-journal-whills fa-lg"></i>
                <span>יומן מסע</span>
            </a>
  ...
  <script type="module">
    ...
    import { getFirestore, doc, onSnapshot, setDoc, updateDoc, arrayUnion, arrayRemove, getDoc, enableIndexedDbPersistence, collection, serverTimestamp, deleteDoc, addDoc, query, orderBy } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
    ...
    // --- Global State ---
    let db, auth, tripDocRef, locationsColRef, journalColRef;
    let localTripData = {}; 
    ...
    function getInitialTripData() {
        ...
        return {
            ...
            totalBudget: 1500, expenses: [], documents: [], gallery: [],
            theme: 'dark',
    ...
    async function fetchWeather() {
        try {
            ...
            // Check for snow/freezing conditions ONLY if temperature is low
            ...
            // If no significant precipitation, base on temperature
    ...
    window.removeTranslationHistoryItem = (event, index) => {
        event.stopPropagation(); // Prevent reuseTranslation from firing
    ...
    async function generateTripSummary() {
        ...
        const { plan, expenses } = localTripData;
        ...
        if (journalColRef) {
            ...
            querySnapshot.forEach((doc) => {
                journalEntries.push(doc.data().text);
            });
        }
    ...
    window.optimizeDayRoute = async (dayId) => {
        ...
        while (activities.length > 0) {
            ...
            if (nearestIndex > -1) {
                ...
                break; // Should not happen if activities array is not empty
            }
        }
    ...
    async function toggleLocationSharing() {
        ...
        if (watchingLocation) {
            ...
            // The important part: delete the location from the database
            if (currentUserId) {
                const userLocationDocRef = doc(locationsColRef, currentUserId);
    ...
    async function deleteJournalEntry(entryId) {
        if (!journalColRef || !entryId) return;
        try {
            const entryDocRef = doc(db, "milantrip", tripId, "journal", entryId);
            await deleteDoc(entryDocRef);
        } catch (error) {
            console.error("Error deleting journal entry: ", error);
            showAlert('שגיאה', 'לא ניתן היה למחוק את הרשומה מהיומן.');
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        window.deleteJournalEntry = deleteJournalEntry; // Expose the new delete function
        ...
        localStorage.setItem('userName', $('#user-name-input').value);
        ...
        onAuthStateChanged(auth, (user) => {
          if (user) {
            ...
            journalColRef = collection(db, "milantrip", tripId, "journal");
            
            initLiveMap();
            listenForLocationUpdates();
            
            // New, separate listener for the journal
            const q = query(journalColRef, orderBy("createdAt", "desc"));
            onSnapshot(q, (snapshot) => {
                const container = $('#journal-entries-container');
                if (snapshot.empty) {
                    container.innerHTML = '<p class="text-center text-gray-500">עדיין אין רשומות ביומן. כתבו משהו!</p>';
                    return;
                }
                container.innerHTML = snapshot.docs.map(doc => {
                    const entry = doc.data();
                    const date = entry.createdAt ? new Date(entry.createdAt.toDate()).toLocaleString('he-IL', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '...';
                    return `
                    <div class="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-lg relative group">
                        ...
                        <button class="absolute top-2 left-2 text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity" onclick="window.deleteJournalEntry('${doc.id}')">
                            <i class="fa-solid fa-trash-alt fa-xs"></i>
                        </button>
                    </div>`;
                }).join('');
        ...
        // New Journal Form Listener
        const journalForm = $('#journal-form');
        journalForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const input = $('#journal-input');
            const text = input.value.trim();
            if (text && journalColRef) {
                try {
                    await addDoc(journalColRef, {
                        text: text,
                    ...
                    showAlert('שגיאה', 'לא ניתן היה לשמור את הרשומה ביומן.');
                }
            }
        });
        ...
        $('#alert-modal-close').addEventListener('click', () => {
             ...
             $('#alert-modal-copy-btn').classList.add('hidden');
        });
        ...
        $('#open-master-map-btn').addEventListener('click', () => {
            ...
            Object.values(localTripData.plan).forEach(day => {
                day.activities.forEach(act => {
                    if(act.address && !locations.some(l => l.includes(act.address))) {
                        locations.push(encodeURIComponent(act.address));
                    }
                });
            });
  ...
  <script>
    // PWA Service Worker Registration Script - Placed outside the module
